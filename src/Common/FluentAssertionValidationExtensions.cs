...
public static class ValidationExceptionAssertionExtensions
{
    ...
    {
        return exception is ValidationException && exception.Message.StartsWith($"Validation failed: {Environment.NewLine}");
    }
    ...
    private static bool IsValidationExceptionForMessage(Exception exception, string expectedMessage)
    {
        ...
        return exception.Message.StartsWith($"Validation failed: {Environment.NewLine}") && exception.Message.EndsWith(expectedMessage);
    }
}